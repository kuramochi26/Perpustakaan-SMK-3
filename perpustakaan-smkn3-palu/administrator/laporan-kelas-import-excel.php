...
header("content-disposition: attachment; filename=laporan-peminjaman-buku-kelas-smkn3-($tanggal_mulai-$tanggal_selesai).xls");
...
$semuadata=array();
	...
	$ambil=mysqli_query($koneksi,"SELECT * FROM transaksi_kelas LEFT JOIN buku ON transaksi_kelas.id_buku=buku.id_buku LEFT JOIN kelas ON transaksi_kelas.id_kelas=kelas.id_kelas LEFT JOIN jurusan ON transaksi_kelas.id_jurusan=jurusan.id_jurusan WHERE status='Dikembalikan' AND tanggal_pinjam BETWEEN '$tanggal_mulai' AND '$tanggal_selesai'");
...
<table border="1">
	...
	<tr>
				...
				<td><?php echo ucwords($value['nama_peminjam']) ?></td>
